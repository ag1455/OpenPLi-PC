...
class MyHandler(BaseHTTPRequestHandler):
   ...
   """
    def do_HEAD(self):
        pass#print "XBMCLocalProxy: Serving HEAD request..."
        #
   ...
   """
    def do_GET(s):
        pass#print "XBMCLocalProxy: Serving GET request..."
    ...
    def answer_request(self, sendData):
        ...
        try:
            ...
            pass#print "In answer_request self.path =", self.path
            request_path=self.path[1:] 
            pass#print "In answer_request request_path =", request_path  
            """    
            request_path=re.sub(r"\?.*","",request_path)
            """
            #If a request to stop is sent, shut down the proxy
            pass#print "In answer_request request_path 2=", request_path     
            if request_path.lower()=="stop":# all special web interfaces here
                sys.exit()
                return
            if request_path.lower()=="favicon.ico":
                pass#print 'dont have no icone here, may be in future'
                self.wfile.close()
                return
            pass#print "In answer_request request_path 3=", request_path     
            ...
            pass#print 'simpledownloaderxxxxxxxxxxxxxxx',simpledownloader
            if streamtype=='' or streamtype==None or streamtype=='none': streamtype='HDS'

            if streamtype=='HDS':

                pass#print 'Url received at proxy',url,proxy,use_proxy_for_chunks,maxbitrate
                ...
                pass#print 'PROXY DATA',downloader.live,enableSeek,requested_range,downloader.total_frags,srange, erange
                ...
                if enableSeek:
                    ...
                    self.send_header("Content-Type", rtype)
                    self.send_header("Accept-Ranges","bytes")
                    pass#print 'not LIVE,enable seek',downloader.total_frags
    ...
    #                crange="bytes "+str(srange)+"-" +str(int(totalsize-1))+"/"+str(totalsize)#recalculate crange based on srange, portionLen and content_size 
                    ...
                    pass#print srange/inflate,erange/inflate,totalsize/inflate
                ...
                if not downloader.init(self.wfile,url,proxy,g_stopEvent,maxbitrate):
                    pass#print 'init throw error because init'#throw error because init
                ...
                if not downloader.init(self.wfile,url,proxy,g_stopEvent,maxbitrate):
                    pass#print 'cannot init but will continue to play'
            ...
            if sendData:
                ...
                pass#print 'srange,framgementToSend',srange,framgementToSend
    ...
    def decode_url(self, url):
        ...
        try:
            ...
            if simpledownloader.lower()=='true':
                pass#print 'params[simpledownloader][0]',params['simpledownloader'][0]
...
class f4mProxy():
    ...
    def start(self,stopEvent,port=PORT_NUMBER):
        ...
        global HOST_NAME
        global g_stopEvent
        pass#print 'port',port,'HOST_NAME',HOST_NAME
        ...
        pass#print "XBMCLocalProxy Starts - %s:%s" % (HOST_NAME, port)
        while(True and not stopEvent.isSet()):
            httpd.handle_request()
        httpd.server_close()
        pass#print "XBMCLocalProxy Stops %s:%s" % (HOST_NAME, port)
...
class f4mProxyHelper():
    ...
    def playF4mLink(self,url,name,proxy=None,use_proxy_for_chunks=False, maxbitrate=0, simpleDownloader=False, auth=None, streamtype='HDS',setResolved=False,swf=None , callbackpath="",callbackparam="", iconImage="DefaultVideo.png", direct="yes"):
##        try:
          ...
          if direct == "yes":
            ...
            try:
                if streamtype==None or streamtype=='' or streamtype in ['HDS'  'HLS','HLSRETRY']:
                    ...
                    listitem.setContentLookup(False)
            except: pass#print 'error while setting setMimeType, so ignoring it '
...
class MyPlayer (xbmc.Player):
    ...
    def play(self, url, listitem):
        pass#print 'Now im playing... %s' % url
        self.stopPlaying.clear()
        xbmc.Player( ).play(url, listitem)

    def onPlayBackEnded( self ):
        # Will be called when xbmc stops playing a file
        pass#print "seting event in onPlayBackEnded "
        self.stopPlaying.set();
        pass#print "stop Event is SET"
    def onPlayBackStopped( self ):
        # Will be called when user stops xbmc playing a file
        pass#print "seting event in onPlayBackStopped "
        self.stopPlaying.set();
        pass#print "stop Event is SET"
