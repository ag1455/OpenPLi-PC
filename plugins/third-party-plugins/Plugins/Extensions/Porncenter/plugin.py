# coding: utf-8
"""
    This file is part of Plugin Porncenter pcd@xtrend-alliance.com
    Copyright (C) 2014
...
try:
       from Plugins.Extensions.Porncenter.lib.Utils import *
except:
...
try:
       from Plugins.Extensions.Porncenter.lib.SkinLoader import loadPluginSkin
except:
...
try:
#       from Plugins.Extensions.Porncenter.lib.VirtualKeyBoard import VirtualKeyBoard
       from Plugins.Extensions.Porncenter.lib.Spinner import Spinner
except:
...
def _(txt):
	...
	if t == txt:
		print "[PorncenterA] fallback to default translation for", txt
...
THISPLUG = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter"
THISADDON = ""
HANDLE = 1
NEWDEFPY = ""
ARG = " "
...
def dellog():###to clear log files
   ...
   if os.path.exists("/tmp/Porncenter_log"):
      os.remove("/tmp/Porncenter_log")
   if os.path.exists("/tmp/Porncenter_error"):
      os.remove("/tmp/Porncenter_error")
...
def getpics(names, pics, tmpfold, picfold):
              if config.plugins.porncenter.skinres.value == "fullhd":
                    nw = 300
              else:
                    nw = 200
              pix = []
              if config.plugins.porncenter.thumb.value == "False":
                      if config.plugins.porncenter.skinres.value == "fullhd":
...
#                          picf = picfold + "/" + name + ".jpg"
                   ...
                   if not fileExists(picf):
                       print "In getpics not fileExists(picf)"
                       if THISPLUG in url:
                          try:
                                  cmd = "cp " + url + " " + tpicf
                          ...
                          try:
                             ...
                             #     print "p =", p
                              ...
                              try:
                                  print "Going in urlopen url except=", url
                                  f = getUrlresp2(url, url)
                                  print "f except =", f
                                  p = f.read()
                                  f1=open(tpicf,"wb")
                                  f1.write(p)
...
config.plugins.porncenter.tempdel = ConfigYesNo(False)
config.plugins.porncenter.picdel = ConfigYesNo(False)
config.plugins.porncenter.subtitle = ConfigYesNo(False)
config.plugins.porncenter.elog= ConfigYesNo(False)
config.plugins.porncenter.cachefold = ConfigText("/media/hdd", False)
config.plugins.porncenter.vlcip = ConfigText("192.168.1.1", False)
config.plugins.porncenter.mmenu = ConfigYesNo(True)
#config.plugins.porncenter.debug = ConfigYesNo(False)
config.plugins.porncenter.thumb = ConfigSelection(default = "False", choices = [("True", _("yes")),("False", _("no")), ("Single", _("single"))])   
config.plugins.porncenter.backup = ConfigYesNo(False)
config.plugins.porncenter.directpl = ConfigYesNo(True)
config.plugins.porncenter.textcol = NoSave(ConfigSelection(default = "0xffffff", choices = [("0xffffff", _("white")), ("0xb3b3b9", _("grey"))]))  
config.plugins.porncenter.textsel = NoSave(ConfigSelection(default = "0xf07655", choices = [("0xf07655", _("red")),("0xe5b243", _("yellow")), ("0xffffff", _("white"))]))   
config.plugins.porncenter.textfont = ConfigSelection(default = "22", choices = [("25", _("medium")), ("35", _("large")), ("22", _("small"))])  
config.plugins.porncenter.skinres = ConfigSelection(default = "hd", choices = [("fullhd", _("Full-HD 1920x1080")), ("hd", _("HD 1280x720"))])  
config.plugins.porncenter.wait = ConfigSelection(default = "10", choices = [("10", _("10sec")),("20", _("20sec")),("30", _("30sec")),("40", _("40sec")),("60", _("60sec")), ("120", _("120sec")), ("180", _("180sec")), ("240", _("240sec")), ("300", _("300sec"))])  
config.plugins.porncenter.listcol = NoSave(ConfigSelection(default = "0x000000", choices = [("0x40000000", _("black")), ("0x40002d39", _("blue"))]))  
config.plugins.porncenter.mainback = NoSave(ConfigSelection(default = "default", choices = [("default", _("default")), ("skin2", _("skin2"))]))  
config.plugins.porncenter.viewdownloads = NoSave(ConfigSelection(default="disabled", choices = [("disabled",_("Not assigned")), ("help",_("Key help"))]))
config.plugins.porncenter.youtubequal = NoSave(ConfigSelection(default="2", choices = [("1",_("SD")), ("2",_("720p")), ("3",_("1080p"))]))

def update_xbmc_text(addon_id):##mfaraj to update the file from xbmc client to be used by xbmc library files
            if addon_id==None:
               return
            try:cachefolder=config.plugins.porncenter.cachefold.value
...
#                        getConfigListEntry(_("YouTube video quality"), cfg.youtubequal),
	...
	def save(self):
                self.saveAll()
                picfold = config.plugins.porncenter.cachefold.value + "/xbmc/pic"
                cmd = "rm -rf " + picfold
                os.system(cmd)
                subs = config.plugins.porncenter.subtitle.value
                tfile = THISPLUG + "/scripts/script.module.SubsSupport"
                if not os.path.exists(tfile):
                    if subs is True:
                       fdest = THISPLUG + "/scripts"
                       dest = "/tmp/subssupport.zip"
                       xfile = "http://www.turk-dreamworld.com/bayraklar/Receiverler/Dreambox/TDW/e2/addons/Porncenter/Script-modules/kodi/script.module.SubsSupport-1.0.0.zip"
...
########################
                ...
                self.timecount = 0
                ncount = config.plugins.porncenter.wait.value 
...
#######################pictures#############
                        ...
                        if "plugin://plugin.video.youtube/" in url:
                               url1 = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/plugins/plugin.video.youtube/default.py"
...
#                self.close()
    ...
    def stream(self):
                ...
                self.picfold = config.plugins.porncenter.cachefold.value+"/xbmc/pic"
                self.tmpfold = config.plugins.porncenter.cachefold.value+"/xbmc/tmp"
...
#                cmd = cmd.replace(")", "\\)")
                ...
                arg2 = args[2]
                arg3 = args[3]
                arg4=config.plugins.porncenter.cachefold.value
...
#                        self.close()
                    ...
                    if config.plugins.porncenter.thumb.value == "True":
                               self.tmppics = getpics(self.names, self.pics, self.tmpfold, self.picfold)
...
#           dellog()
        ...
        def showerror(self):
          try:
                 from Plugins.Extensions.Porncenter.lib.XBMCAddonsinfo import XBMCAddonsinfoScreen
          except:
...
#                            from lib.VirtualKeyBoard import VirtualKeyBoard
            ...
            if itype == 0:
              ...
              if name == "Setup":###to generate e2 e2sett.py
                ...
                try:
                       from Plugins.Extensions.Porncenter.lib.XBMCAddonsSetup import AddonsettScreen
                except:       
                ...
                favorites_xml="/etc/Porncenter/favorites.xml"
                import os
                if not os.path.exists(favorites_xml):
                       try:
                        if not os.path.exists("/etc/Porncenter"):
                           os.makedirs("/etc/Porncenter")
                        copyfile(THISPLUG+"/lib/defaults/favorites.xml",favorites_xml)
                       except:
                        return
                try:
                       from Plugins.Extensions.Porncenter.lib.favorites import getfavorites
                except:
...
class XbmcPluginScreen(Screen):
    ...
    def __init__(self, session, name, names, urls, tmppics,curr_run):
		Screen.__init__(self, session)
		if curr_run == 1:
		       self.skinName = "KodiMenusScreenB2"
		else:
...
#                self.names1 = names
                ...
                ####################################  
#screen number4self.urls1 = [' ', '/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/plugins/plugin.video.IPTVupdater/default.py?plugin://plugin.video.IPTVupdater/?url=+&name=Information&showtext=true&mode=3']
...
#		       "blue": self.addon,
    ...
    def delete(self):
           ...
           xmlfile = "/etc/Porncenter/favorites.xml"
...
##################################
    def checkfixsh(self):
                ...
                if not self.name in self.fixlist:
                        ...
                        fdest = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/scripts"
...
#                self.checkfix() Not necessary as okclick2 calls it.
    ...
    def installB(self, fplug):
                fdest = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/plugins"
                    ...
                    for item in items:
                        ...
                        if item == "":
                             ...
                             if "plugin.video." in item:
                                    self.fdest = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/plugins"
                             else:
                                    self.fdest = "/usr/lib/enigma2/python/Plugins/Extensions/Porncenter/scripts"
                             self.dest = "/tmp/" + plug
...
########################################
    ...
    def runE2plug(self, name):
           from Plugins.Extensions.Porncenter.Execlist import Execlist
...
def main(session, **kwargs):
        ...
        log = config.plugins.porncenter.elog.value
        if log is True:
        ...
        if not os.path.exists("/etc/Porncenter"):
               system("opkg update")
               system("mkdir -p /etc/Porncenter")
        if not os.path.exists("/etc/Porncenter/favorites.xml"):
               cmd = "cp " + THISPLUG + "/lib/defaults/favorites.xml /etc/Porncenter/"
               system(cmd)
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/vid")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/pic")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/tmp")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/home/")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/database/")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/userdata/")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/userdata/Database/")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/home/addons/")
        system("mkdir -p "+ config.plugins.porncenter.cachefold.value+"/xbmc/home/addons/packages")
        print "In def main 4"
...
def start():
        ...
        if os.path.exists(gpath):
              cmd = "rm -rf " + gpath
              os.system(cmd)
        _session.open(StartPlugin_mainmenu)

def mpanel(menuid, **kwargs):
	if menuid == "mainmenu":
		return [("Porncenter", main, "xbmc_addons", 12)]
	else:
		return []
...
def Plugins(**kwargs):
	loadPluginSkin(kwargs["path"])
        try:
          viewdownloads=config.plugins.porncenter.viewdownloads.value
          mmenu=config.plugins.porncenter.mmenu.value
	except:
          viewdownloads='disabled'
          mmenu=False
	list = []

#        list.append(PluginDescriptor(icon="plugin.png",name="Porncenter", description="Adult 18+ videos from internet sites", where = PluginDescriptor.WHERE_MENU, fnc=mpanel))
        list.append(PluginDescriptor(icon="plugin.png",name="Porncenter", description="Adult 18+ videos from internet sites", where = PluginDescriptor.WHERE_PLUGINMENU, fnc=main))
        list.append(PluginDescriptor(icon="plugin.png",name="Porncenter", description="Adult 18+ videos from internet sites", where = PluginDescriptor.WHERE_EXTENSIONSMENU, fnc=main))
...
class classJobManagerViews():
	...
	def gotSession(self, session):
        ...
        def ShowHide(self):
#            from  Plugins.Extensions.Porncenter.lib.download import viewdownloads
#            viewdownloads(self.session,THISPLUG)
            try:
                   from Plugins.Extensions.Porncenter.lib.XBMCAddonsMediaExplorer import XBMCAddonsMediaExplorer
            except:
