...
class Satloader(Screen):
	...
	def onLayoutFinished(self):
		self.list.clearList()
		self.list.addSelection("Satbeams", "http://satellites.satloader.net/satbeams", 0, None)
		self.list.addSelection("Kingofsat", "http://satellites.satloader.net/kingofsat", 1, None)
		self.list.addSelection("Kingofsat (feeds)", "http://satellites.satloader.net/kingofsat/feeds", 2, None)
#		self.list.addSelection("Lyngsat", "http://satellites.satloader.net/lyngsat", 3, None)
...
class SatloaderAbout(Screen):
	...
	def __init__(self, session):
		Screen.__init__(self, session)
		self["info"] = Label(_("Satloader Plugin\nAuthor: Ismail Demir\nwww.satloader.net"))
...
def Plugins(**kwargs):
	return [
		PluginDescriptor(name=_("Satloader Plugin"), description=_("updates satellites.xml"), icon="satloader.png", where = PluginDescriptor.WHERE_PLUGINMENU, fnc=main),
		PluginDescriptor(name=_("Satloader Plugin"), description=_("updates satellites.xml"), where = PluginDescriptor.WHERE_EXTENSIONSMENU, fnc=main)
		]
