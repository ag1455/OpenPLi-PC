...
def localeInit():
    ...
    gettext.bindtextdomain("AntiScrollbar", resolveFilename(SCOPE_PLUGINS, "Extensions/AntiScrollbar/locale"))

def _(txt):
    t = gettext.dgettext("AntiScrollbar", txt)
    if t == txt:
        print "[AntiScrollbar] fallback to default translation for", txt
...
def Plugins(**kwargs):
  return [PluginDescriptor(name=myname,description=_("overlay for scrolling bars"),where = PluginDescriptor.WHERE_PLUGINMENU,fnc = main, icon="plugin.png"),
