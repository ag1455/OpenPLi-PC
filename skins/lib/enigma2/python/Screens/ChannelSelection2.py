...
class ChannelContextMenu(Screen):
			...
			if not inBouquetRootList:
				...
				if isPlayable:
					if config.servicelist.startupservice.value == self.csel.getCurrentSelection().toString():
						append_when_current_valid(current, menu, (_("reset startup service for booting"), self.unsetStartupService), level = 0)
					else:
						append_when_current_valid(current, menu, (_("set as startup service after booting"), self.setStartupService), level = 0)
					if config.eleavingstandbymode.startupservice.value == self.csel.getCurrentSelection().toString():
						append_when_current_valid(current, menu, (_("reset startup service for leaving standby mode"), self.unsetStartupServiceEleavingstandby), level = 0)
					else:
						append_when_current_valid(current, menu, (_("set as startup service after leaving standby mode"), self.setStartupServiceEleavingstandby), level = 0)
	...
	def setStartupServiceEleavingstandby(self):
		config.eleavingstandbymode.startupservice.value = self.csel.getCurrentSelection().toString()
		path = ''
		for i in self.csel.servicePath:
			path += i.toString()
			path += ';'
		config.eleavingstandbymode.startuproot.value = path
		config.eleavingstandbymode.startupmode.value = config.eleavingstandbymode.lastmode.value
		config.eleavingstandbymode.save()
		configfile.save()
		self.close()

	def unsetStartupServiceEleavingstandby(self):
		config.eleavingstandbymode.startupservice.value = ''
		config.eleavingstandbymode.save()
		configfile.save()
		self.close()	
...
class ChannelSelectionBase(Screen):
	def __init__(self, session):
		Screen.__init__(self, session)
		self.setup_title = _("Channel Selection")
	...
	def showSatellites(self):
		if not self.pathChangeDisabled:
#--- vlamo hack start -- #3 ( add HD-channels list ) -----------------------------------------------------------------------------------------------------------
...
#--- vlamo hack end -- #3 ---------------------------------------------------------------------------------------------------------------------------------------
					...
					if hdref:
						hdref.setName("%s - %s" % ("Satellites", _("Services")))
...
config.eleavingstandbymode = ConfigSubsection()
config.eleavingstandbymode.lastmode = ConfigText(default = "tv")
config.eleavingstandbymode.startupservice = ConfigText()
config.eleavingstandbymode.startuproot = ConfigText()
config.eleavingstandbymode.startupmode = ConfigText(default = "tv")
...
#--- vlamo hack start -- ( use startup service on leave standby ) -----------------------------------------------------------------------------------------------
		config.misc.standbyCounter.addNotifier(self.standbyCounterCallback, initial_call = False)
#--- vlamo hack end -- ( use startup service on leave standby ) -------------------------------------------------------------------------------------------------
...
#--- vlamo hack start -- ( use startup service on leave standby ) -----------------------------------------------------------------------------------------------
	def standbyCounterCallback(self, ConfigElement):
		if config.eleavingstandbymode.startupservice.value and config.eleavingstandbymode.startuproot.value:
			config.eleavingstandbymode.lastmode.value = config.eleavingstandbymode.startupmode.value
			if config.eleavingstandbymode.lastmode.value == "tv":
				config.tv.lastservice.value = config.eleavingstandbymode.startupservice.value
				config.tv.lastroot.value = config.eleavingstandbymode.startuproot.value
				self.setModeTv()
			elif config.eleavingstandbymode.lastmode.value == "radio":
				config.radio.lastservice.value = config.eleavingstandbymode.startupservice.value
				config.radio.lastroot.value = config.eleavingstandbymode.startuproot.value
				self.setModeRadio()

			ref = self.getCurrentSelection()
			if ref and ref.valid():
				from Screens.Standby import inStandby
				inStandby.prev_running_service = ref
#--- vlamo hack end -- ( use startup service on leave standby ) -------------------------------------------------------------------------------------------------
